...
def capacity_audit():
    ...
    try:
        ...
        # IBM lsvdisk list returns capacity="" for VG-member volumes.
        # A per-object POST /rest/v1/lsvdisk/<id> returns the real value.

        # Collect the vdisk names we need: VG members on DR + their source twins
        vg_member_names: set = set()
        for vols in dr_volumes_by_vg.values():
            for v in vols:
                vg_member_names.add(v.get('name', ''))

        # IBM lsvdisk list returns capacity="" or "0" for VG-member volumes
        def _needs_enrichment(rec):
            cap = rec.get('capacity', '') if rec else ''
            return not cap or cap == '0'

        src_idx_raw = CapacityAuditor.build_vdisk_index(src_vdisks_raw)
        for vname in vg_member_names:
            src_rec = src_idx_raw.get(vname)
            if src_rec and _needs_enrichment(src_rec):
                vid = src_rec.get('id') or vname
                detail = src_client.get_vdisk_detail(vid)
                if detail:
                    src_idx_raw[vname] = detail
                else:
                    logger.warning("src enrichment failed for '%s' (id=%s)", vname, vid)
        ...
        for vname in vg_member_names:
            ...
            if dr_rec and _needs_enrichment(dr_rec):
                vid = dr_rec.get('id') or vname
                detail = dr_client.get_vdisk_detail(vid)
                if detail:
                    dr_idx_raw[vname] = detail
                else:
                    logger.warning("dr enrichment failed for '%s' (id=%s)", vname, vid)
