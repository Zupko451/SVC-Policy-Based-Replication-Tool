...
class IBMStorageVirtualizeClient:
    ...
    def get_vdisk_detail(self, vdisk_id: str) -> Optional[Dict]:
        ...
        IBM SV REST API: POST /rest/v1/lsvdisk/<id>  (no request body).
        The detail response includes the real 'capacity' field (in bytes as a
        string, e.g. '107374182400') that is absent from the list response for
        VG-member volumes.
        """
        data = self._make_request(
            f"{self.VDISK_ENDPOINT}/{vdisk_id}", method="POST"
        )
        if data is None:
            return None
...
class CapacityAuditor:
    ...
    # IMPORTANT: 'B' must be last — endswith('B') matches 'KB', 'MB', 'GB',
    # 'TB', 'PB' too, so multi-char suffixes must be checked first.
    ...
    def audit_volume_capacity_delta(
        ...
        capacity against the DR target provisioned capacity, and include the
        used (written) capacity from both sides.

        Returns a list of per-volume dicts:
          vg_name, vol_name,
          source_capacity_gib, dr_capacity_gib,
          source_used_gib, dr_used_gib,
          delta_gib,   (DR provisioned − source provisioned; negative = DR undersized)
          status       ('ok' | 'mismatch' | 'missing')

        IBM SV field names:
          capacity      — provisioned size (thin or thick)
          used_capacity — bytes actually written / allocated on the backend
        """
        results = []
        for vg in vg_names:
            for vol in dr_volumes_by_vg.get(vg, []):
                ...
                dr_used  = CapacityAuditor._to_gib(dr_rec.get('used_capacity')  if dr_rec  else None)
                src_used = CapacityAuditor._to_gib(src_rec.get('used_capacity') if src_rec else None)
